...
Room operations and connection transitions use the Host `ctx.notifications.show()` service. Source icon/name,
...
Protocol source and hashes are recorded in [notification-sdk-evidence.json](../../client/notification-sdk-evidence.json).
...
into a fresh directory, then run from the repository root:
