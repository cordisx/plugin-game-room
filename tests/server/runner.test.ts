...
const run = async (rules: string, method: 'setup' | 'observe' = 'setup', limits = {}) =>
  await invoke({ rules, method, args: [], ctx, seed: 'fixed-server-secret', cursor: 0 }, limits)
await test('WASM guest has no filesystem, network, process, Date or random; deterministic platform random', async () => {
  const source =
...
await test('CPU, memory, output and async abuse fail closed; later healthy guest survives', async () => {
  for (
    const source of [
...
await test('observation cannot advance platform randomness or import host modules', async () => {
