...
await test('SIGKILL discards uncommitted writes and retains acknowledged action, session and idempotency', async (t) => {
  ...
  assert.equal((await app.accounts.authenticate(message.token)).id, message.accountId)
  const restored = await app.engine.load(message.roomId)
  ...
  const replay = await app.engine.serial(async () =>
    await app.engine.action(message.accountId, message.roomId, message.input)
  )
  assert.equal(replay.version, message.version)
  assert.equal((await app.engine.load(message.roomId)).version, message.version)
})
