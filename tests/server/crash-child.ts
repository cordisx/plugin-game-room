...
await h.app.engine.serial(async () =>
  await h.app.engine.action(h.alice.account.id, room.id, {
    expectedVersion: room.version,
    idempotencyKey: 'durable-action',
    action: { type: 'move' },
  })
)
const committed = await h.app.engine.load(room.id)
...
await h.app.store.db.prepare('UPDATE rooms SET body=? WHERE id=?').run('{}', room.id)
process.send!({
  roomId: room.id,
